const ScoreTopPart = ({ iscore }) => {
  console.log(iscore?.status);
  return (
    <div className={` w-full  bg-transparent px-0 l `}>
      <div className=" flex w-full justify-between items-center px-3.5 py-1  shadow-md  my-1">
        <div className=" flex items-start justify-start w-max flex-col max-w-[70%]">
          <div className="  text-start text-white  font-bold text-sm flex items-start justify-start w-full">
          ...
          <div className=" flex items-center justify-start gap-x-1">
            ...
            <div className=" flex items-center justify-center rounded-[4px] px-1.5 py-0.5 bg-primary">
        ...
        <div className=" flex flex-col gap-y-[3px] text-end max-w-[60%] text-white w-full">
          <span className="  font-bold leading-6 text-lg ">
            {iscore?.status}
          </span>
          <div className=" text-x font-semibold text-end leading-3">
            {iscore?.crr && (
              <span className="text-white ">CRR : {iscore?.crr}</span>
            )}
            {iscore?.rrr && (
              <span className=" ml-2 text-white ">RR : {iscore?.rrr}</span>
            )}
          </div>
          {/* <span className=" text-xs text-white w-full text-end">
        ...
        <div className=" px-[11px] relative bg-bg_color_primary w-full   shadow-md z-10">
          <div className=" w-full flex items-center py-1 justify-start overflow-scroll no-scrollbar gap-x-2">
            {iscore?.currentOver?.length > 0 && (
              ...
              >
                <span className=" w-max min-w-11 text-xs font-medium text-white">
                  Current Over
                </span>
            ...
            <div className=" flex items-center justify-center text-xs gap-x-0.5 font-medium leading-4 border-r border-border_color_primary pr-3 text-white">
              <span>=</span>
            ...
            <div className=" flex items-center justify-center text-xs gap-x-0.5 font-medium leading-4 border-r border-border_color_primary pr-5 text-white">
              <span>=</span>
